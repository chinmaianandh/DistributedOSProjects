[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/pRECeOUJ)
...
Isha Nilesh Gohel - igohel@umass.edu

Roshini Sanikop - rsanikop@umass.edu

Work Division : 

Part - 1 implementation was divided among two. The catalog service and the frontend service was written by Isha. The order service and the client was written by Roshini. 

Part -2 was written by Roshini.

Part - 3 was implemented by both. Writing the test cases and testing of the application was done by Isha. Evaluation document was written by Roshini.

Instructions on how to run the application have been written in the readme file in the src directory.
...
service and a stock order service.
...
In an HTTP-based REST API, a client sends its request as an HTTP request and receives a reply as a
...
assign them to a thread pool. You can use any builtin thread pool to service client requests.
...
sent back to the client as an HTTP reply.
...
functionality of lab 2 and provide higher-level abstractions to developers. The goal here is to
...
file on disk ("database"). The disk file will persist in the state of the catalog. When the service
...
the catalog. It will also increment or decrease the number of stocks available for sale, depending 
...
The catalog service is implemented as a server that listens to requests from the front-end service or
...
describe how you implemented your interface in the interfaces section of your design doc.

Like the front-end server, you should employ threads to service incoming requests. Since the catalog
...
If the order was successful, the order service generates a transaction number and returns it to the
front-end service. The transaction number should be a unique, incremental number, starting from 0.
The order service also needs to maintain the order log (including transaction number, stock name,
...
(i.e., a list of functions and their input/outputs). You can use any method for front-end to invoke
...
**Note: To maintain modularity, the order service should not simply forward buy requests to the catalog service—order processing logic should remain within the order service. Additionally, using two separate calls to the catalog service (one to check item availability and another to increase/decrease stock) can lead to race conditions if only one item is left in stock. To prevent this, the order service should first send an increase/decrease request to the catalog service and determine whether the order was successful based on the response.**


### Client

The client in this lab works in the following way. First it opens an HTTP connection with the
...
First, create a dockerfile for each of the three microservices you implemented in part 1.
...
**Note: You may use the course cs677-docker for development purposes. However, you are required to write your own Dockerfile for each microservice. Only the client in this part is permitted to use the course Docker image.**
...
First, write some simple test cases to verify that your code works as expected. Be sure to test the
code and error handling (e.g., by looking up stocks that do not exist or buying stocks with
...
expected. Submit your test cases and the outputs in a test directory.

Second, write some performance/load tests to evaluate the performance of your application. Deploy
more than one client process and have each one make concurrent requests to the server. The
clients and all microservices should be running on separate containers. Measure
...
plots showing the number of clients on the X-axis and response time/latency on the Y-axis.
...
`build.sh` script that can build all your images.
...
Submit your test cases in the test directory. Attach sample output of test cases on the docs
directory.
...
1) Part 1 is 50% of the lab grade.
    ...
    * Clear and full description of interfaces/API, including inputs and outputs, of each service (5%)
...
and should be submitted through Piazza (use the exceptionRequests folder in Piazza). Four free late
